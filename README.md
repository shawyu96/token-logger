...
| **所有对话** | 分页 5 条，按最后活跃时间倒序排列，含创建时间列（带年），点选查看详情 |
| **会话详情弹窗** | 每次调用的：序号（01-99）、时间、模型、输入/输出/缓存、耗时（动态单位 ms/s/m），最新在前倒序排列 |
| **全局筛选栏** | 时间范围（7天/30天/全部）+ 模型精确匹配（支持多模型会话） |
| **↻ 刷新** | 全屏加载动画，数据重新拉取 |
| **吸顶表头** | 标题/筛选栏/统计卡片滚动时固定在顶部，不随数据滚动消失 |
